...
function insertAnActivityLog($pdo, $operation, $id, $address, 
		$specialization, $email, $username) {

	$sql = "INSERT INTO activity_logs (operation, id, address, 
		specialization, email, username) VALUES(?,?,?,?,?,?)";

	$stmt = $pdo->prepare($sql);
	$executeQuery = $stmt->execute([$operation, $id, $address, 
		$specialization, $email, $username]);
...
function insertNewCoach($pdo, $first_name, $last_name, $email, $gender, $address, $specialization, $years_of_experience, $added_by) {
	...
	if ($insertCoach) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getCoachID['id'], 
			$getCoachID['address'], $getCoachID['specialization'], 
			$getCoachID['email'], $_SESSION['username']);
...
function deleteUser($pdo, $id) {
	...
	$insertAnActivityLog = insertAnActivityLog($pdo, "DELETE", $getCoachByID['id'], 
		$getCoachByID['address'], $getCoachByID['specialization'], 
		$getCoachByID['email'], $_SESSION['username']);
